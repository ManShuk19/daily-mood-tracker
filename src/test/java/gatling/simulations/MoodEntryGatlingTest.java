...
public class MoodEntryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all moodEntries").get("/api/mood-entries").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new moodEntry")
                        .post("/api/mood-entries")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"date\": \"2020-01-01T00:00:00.000Z\"" + ", \"mood\": \"HAPPY\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_moodEntry_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created moodEntry").get("${new_moodEntry_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created moodEntry").delete("${new_moodEntry_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MoodEntry entity").exec(scn);
