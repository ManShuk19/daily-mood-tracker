...
export const MoodEntry = () => {
  ...
  return (
    <div>
      <h2 id="mood-entry-heading" data-cy="MoodEntryHeading">
        Mood Entries
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} /> Refresh list
          </Button>
          <Link to="/mood-entry/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
      ...
      <div className="table-responsive">
        {moodEntryList && moodEntryList.length > 0 ? (
            ...
            <tbody>
              {moodEntryList.map((moodEntry, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/mood-entry/${moodEntry.id}`} color="link" size="sm">
                      {moodEntry.id}
                    </Button>
                  </td>
                  <td>{moodEntry.date ? <TextFormat type="date" value={moodEntry.date} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{moodEntry.mood}</td>
                  <td>{moodEntry.user ? moodEntry.user.login : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/mood-entry/${moodEntry.id}`} color="info" size="sm" data-cy="entityDetailsButton">
          ...
          !loading && <div className="alert alert-warning">No Mood Entries found</div>
        )}
      </div>
      {totalItems ? (
